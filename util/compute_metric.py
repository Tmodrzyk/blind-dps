...
def stats(label_root, recon_root):
    ...
    for idx in tqdm(range(11)):
        ...
        lpips_vgg = loss_fn_vgg(torch.tensor(label).unsqueeze(0).unsqueeze(0).to(device), torch.tensor(recon).unsqueeze(0).unsqueeze(0).to(device)).item()
        
        # LPIPS AlexNet
        lpips_alex = loss_fn_alex(torch.tensor(label).unsqueeze(0).unsqueeze(0).to(device), torch.tensor(recon).unsqueeze(0).unsqueeze(0).to(device)).item()
...
recon_root_richarson_lucy = Path(f'/home/modrzyk/code/blind-dps/results/ffhq/fast-refine/{task}/recon/')
